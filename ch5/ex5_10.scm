...
; increment
(define (increment-reg-name inst) (cadr inst))
    ...
    (let ((target (get-register machine (increment-reg-name inst))))
        (lambda ()
            (let ((value (get-contents target)))
                (if (number? value)
                    (begin (set-contents! target (+ value 1))
                           (advance-pc pc))
                    (error "number is required -- MAKE-INCREMENT" value))))))
...
(print (set-register-contents! add-two-machine 'a 200))
(print (start add-two-machine))
(print (get-register-contents add-two-machine 'a))
